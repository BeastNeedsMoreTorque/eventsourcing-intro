...
object DBOperations {
    ...
    jdbcSession.withConnection(
      connection =>
        tryWith(connection.prepareStatement(
          "SELECT time_uuid_offset FROM read_side_offsets WHERE read_side_id = ? AND tag = ?")) {
          statement =>
            {
  ...
  private[impl] def addClub(jdbcSession: JdbcSession,
                            offset: Offset,
                            club: ClubData): Future[Boolean] =
    jdbcSession.withTransaction(
      connection => {
        tryWith(connection.prepareStatement(
          "INSERT INTO league (team, gamesPlayed, points) VALUES (?, ?, ?)")) {
          statement =>
  ...
  private[impl] def updateOffset(connection: Connection,
                                 offset: Offset): Boolean = {
    tryWith(connection.prepareStatement(
      "UPDATE read_side_offsets SET time_uuid_offset = ? WHERE read_side_id = ? AND tag = ?")) {
      statement =>
        statement.setString(1,
                            offset.asInstanceOf[TimeBasedUUID].value.toString)
  ...
  private[impl] def addGame(jdbcSession: JdbcSession,
                            offset: Offset,
                            game: GameData): Future[Boolean] =
    jdbcSession.withTransaction(connection => {
      tryWith(connection.prepareStatement(
        "UPDATE league set gamesPlayed = gamesPlayed + 1, points = points + ? WHERE team = ?")) {
        statement =>
  ...
  private[impl] def revokeResult(jdbcSession: JdbcSession,
                                 offset: Offset,
                                 game: GameData): Future[Boolean] =
    jdbcSession.withTransaction(connection => {
      tryWith(connection.prepareStatement(
